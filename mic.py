...
class Mic:
    ...
    def __init__(self, speaker, lmd, dictd, lmd_persona, dictd_persona, lmd_music=None, dictd_music=None, lmd_num=None, dictd_num=None):
        """
        ...
        if lmd_num and dictd_num:
            self.speechRec_num = ps.Decoder(hmm=hmdir, lm=lmd_num, dict=dictd_num)
    ...
    def transcribe(self, audio_file_path, PERSONA_ONLY=False, MUSIC=False, NUMBER=False):
        """
        ...
        if MUSIC:
            ...
            self.speechRec_num.decode_raw(wavFile)
            result = self.speechRec_num.get_hyp()
        elif PERSONA_ONLY:
    ...
    def activeListen(self, THRESHOLD=None, LISTEN=True, MUSIC=False, NUMBER=False):
        """
        ...
        if NUMBER:
            return self.transcribe(AUDIO_FILE, NUMBER=True)
